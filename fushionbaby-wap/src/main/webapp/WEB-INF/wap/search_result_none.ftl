<!DOCTYPE html>
<html lang="zh-CN">
  <head>
    <meta charset="utf-8">
    <title>Fushionbaby触屏版-搜索结果：空</title>
    <meta name="apple-mobile-web-app-title" content="兜士宝-搜索结果：空">
    <script type="text/javascript" src="js/public-headTag.js"></script><!-- 公共头部便签，css,js等 -->
  </head>
  <body>
    <div class="container">
      <div class="public_header fl width100">
        <div class="public_header_wrap fl width100">
          <a class="public_back" href="javascript:void(0);"  onclick="history.go(-1);">返回</a>
          搜索结果
          <a class="public_btn_r disabled" href="javascript:void(0);"></a>
        </div>
      </div>
      <div class="search_result_none fl">
        <img src="css/images/search_result_none.png" alt="">
        <p>很抱歉，没有您搜索的产品!</p>
      </div>
    </div>
    <script type="text/javascript" src="js/footer.js"></script>
  </body>
</html>