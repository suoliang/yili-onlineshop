...
/*	public static String softwareSerialNo = "6SDK-EMY-6688-KEZOQ";// 软件序列号,请通过亿美销售人员获取
	public static String key = "346466";// 序列号首次激活时自己设定
	
*/
	private TestSDKClient(){
		
	}
	private static final String PWD = "471593";/* 密码,请通过亿美销售人员获取*/
	private static final String[]  MOBILES = new String[] { "15000000000" };
	...
	public static void testRegistEx() {
		try {
			int i = SingletonClient.getClient().registEx(PWD);
	...
	public static void testSendSMS() {
		try {
			int i = SingletonClient.getClient().sendSMS(new String[] { "13817274945" }, "【兜世宝】您好，我是杜习虎", "",5);// 带扩展码
	...
	public static void testSendScheduledSMS() {
		try {
			int i = SingletonClient.getClient().sendScheduledSMSEx(MOBILES,
	...
	public static void testSendSMSAddMessageId() {
		try {
			int i = SingletonClient.getClient().sendSMSEx(MOBILES, "带有信息ID的短信", "", "GBK", 5, 123456789);
			System.out.println("testsSendScheduledSMS=====" + i);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	*/
	...
	public static void testSendVoice() {
		try {
			String value = SingletonClient.getClient().sendVoice(MOBILES, "123456", "", "GBK", 5, System.currentTimeMillis());
	...
	public static void testSerialPwdUpd() {
		try {
			int a = SingletonClient.getClient().serialPwdUpd("123456", PWD);
