...
import com.aladingshop.store.util.bean.FtpInfo;
...
public class FTPUtils {
	...
	public static boolean deleteFile(FtpInfo ftpInfo, String remoteDic, String fileName, String fileType) {
		try {
			// ftp信息的本地相对目录没有精确到文件格式,取文件的后缀名为文件夹名
			FTPClient ftp = connectServer(ftpInfo.getHost(), ftpInfo.getPort(), ftpInfo.getUserName(),
					ftpInfo.getPassword(), fileType);
	...
	public static void uploadFile(Map<String, String> param, UploadResult result) throws Exception {
		String fileName = DateUtil.getCurrentDateStr(DateUtil.FULL_PATTERN) + param.get("filename").replace(" ", ""); // 名字加时间戳
		// 存储上传之后的文件名
		result.setFileNameAfter(fileName);
		// 获取日期字符串
		...
		if (file.exists()) {
			if (!upLoadFile(getFileInfo(param), fileName, fileName, type)) {
				log.error("ftp 异常");
