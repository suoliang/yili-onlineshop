...
import com.aladingshop.store.util.ExportExcelUtils;
...
public class OrderExcelReport {
	...
	public void getReport(String fileName,List<OrderDto> orderList,HttpServletResponse response) throws IOException{
		...
		for(OrderDto order:orderList){
			...
			ExportExcelUtils.createCell(row,6, centerStyle,cellType,order.getOrderStatus());//订单状态
			ExportExcelUtils.createCell(row,7, centerStyle,cellType,order.getReceiver());//收货人
			ExportExcelUtils.createCell(row,8, centerStyle,cellType,order.getReceiverMobile());//联系电话
			ExportExcelUtils.createCell(row,9, leftStyle,cellType,order.getMemo());//会员留言
	...
	private void createHead(HSSFWorkbook wb,HSSFSheet sheet){
		...
		ExportExcelUtils.createCell(headRow,6, boldStyle,cellType,"订单状态");
		ExportExcelUtils.createCell(headRow,7, boldStyle,cellType,"收货人");
		ExportExcelUtils.createCell(headRow,8, boldStyle,cellType,"联系电话");
		ExportExcelUtils.createCell(headRow,9, boldStyle,cellType,"会员留言");
