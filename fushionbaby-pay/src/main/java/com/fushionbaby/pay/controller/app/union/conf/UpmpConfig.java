package com.fushionbaby.pay.controller.app.union.conf;

import java.io.InputStream;
import java.util.PropertyResourceBundle;

/**
 * 类名：中国银联UPMP配置类
 * 功能：配置类
 * */
public class UpmpConfig  {
	//版本号
	public static String VERSION;
	//编码方式
	public static String CHARSET;
	//交易网址
	public static String TRADE_URL;	
	//查询网址
	public static String QUERY_URL;
	//商户代码
	public static String MER_ID;
	//通知URL
	public static String MER_BACK_END_URL;	
	//前台通知URL
	public static String MER_FRONT_END_URL;	
	//返回URL
	public static String MER_FRONT_RETURN_URL;	
	//加密方式
	public static String SIGN_TYPE;
	//商城密匙
	public static String SECURITY_KEY;
	
	public static Integer CONNECTION_TIMEOUT;//15000
	public static Integer SO_TIMEOUT;//30000
	public static Boolean STALE_CHECK_ENABLED;//true
	public static Boolean TCP_NO_DELAY;//true
	public static Integer DEFAULT_MAX_CONNECTIONS_PER_HOST;//100
	public static Integer MAX_TOTAL_CONNECTIONS;//1000
	public static String  CONTENT_ENCODING;//utf-8
	
	private static final String KEY_VERSION = "version";
	private static final String KEY_CHARSET = "charset";
	private static final String KEY_TRADE_URL = "upmp.trade.url";
	private static final String KEY_QUERY_URL = "upmp.query.url";
	private static final String KEY_MER_ID = "mer.id";
	//private static final String KEY_MER_BACK_END_URL = "mer.back.end.url";
	//private static final String KEY_MER_FRONT_END_URL = "mer.front.end.url";
	private static final String KEY_SIGN_METHOD = "sign.method";
	//private static final String KEY_SECURITY_KEY = "security.key";
	private static final String http_connection_timeout = "http.connection.timeout";//15000
	private static final String http_so_timeout = "http.so.timeout";//30000
	private static final String http_stale_check_enabled = "http.stale.check.enabled";//true
	private static final String http_tcp_no_delay= "http.tcp.no.delay";//true
	private static final String http_default_max_connections_per_host= "http.default.max.connections.per.host";//100
	private static final String http_max_total_connections= "http.max.total.connections";//1000
	private static final String http_content_encoding = "http.content.encoding";//utf-8
	
	//成功应答码
	public static final String RESPONSE_CODE_SUCCESS = "00";
	//签名
	public final static String SIGNATURE = "signature";
	//签名方法
	public final static String SIGN_METHOD = "signMethod";
	//应答码
	public final static String RESPONSE_CODE = "respCode";
	//应答信息
	public final static String RESPONSE_MSG = "respMsg";
	private static final String CONF_FILE_NAME = "conf/app_union.properties";
	
	static {
        try {
        	
			InputStream fis = UpmpConfig.class.getClassLoader().getResourceAsStream(CONF_FILE_NAME);
			PropertyResourceBundle props = new PropertyResourceBundle(fis);
			VERSION = props.getString(KEY_VERSION);
			CHARSET = props.getString(KEY_CHARSET);
			TRADE_URL = props.getString(KEY_TRADE_URL);
			QUERY_URL = props.getString(KEY_QUERY_URL);
			MER_ID = props.getString(KEY_MER_ID);
			//MER_BACK_END_URL = props.getString(KEY_MER_BACK_END_URL);
			//MER_FRONT_END_URL = props.getString(KEY_MER_FRONT_END_URL);
			SIGN_TYPE = props.getString(KEY_SIGN_METHOD);
			//SECURITY_KEY = props.getString(KEY_SECURITY_KEY);
			CONNECTION_TIMEOUT = Integer.parseInt(props.getString(http_connection_timeout));
			SO_TIMEOUT = Integer.parseInt(props.getString(http_so_timeout));
			STALE_CHECK_ENABLED = Boolean.parseBoolean(props.getString(http_stale_check_enabled));
			TCP_NO_DELAY = Boolean.parseBoolean(props.getString(http_tcp_no_delay));
			DEFAULT_MAX_CONNECTIONS_PER_HOST = Integer.parseInt(props.getString(http_default_max_connections_per_host));
			MAX_TOTAL_CONNECTIONS = Integer.parseInt(props.getString(http_max_total_connections));
			CONTENT_ENCODING = props.getString(http_content_encoding);
        } catch(Exception e) {
        	e.printStackTrace();
        }
    }
	
}
