		$(document).ready(
			function() {
				...
				$("#chuantong").mouseenter(
					function(){
						clearInterval(timer);
						$('.anniu .leftbut').fadeTo(500,0.5)
						$('.anniu .rightbut').fadeTo(500,0.5)
					}
				);
				...
				$("#chuantong").mouseleave(
					function(){
						...
						$('.anniu .leftbut').fadeTo(500,0)
						$('.anniu .rightbut').fadeTo(500,0)
					}
				);
				//******定时器*******
					...
					if(!$(".tuul").is(":animated")){
						if(nowimg < $("#chuantong .tuul li").length - 2){
							...
							$("#chuantong .tuul").animate(
								{
									"left":-1140 * ($("#chuantong .tuul li").length - 1)
								}
								,500
								,function(){
					...
					function(){
						if(!$(".tuul").is(":animated")){
							if(nowimg > 0){
								...
								//先让真狮子瞬间替换为假狮子，然后拉动
								$("#chuantong .tuul").css("left",-1140*($("#chuantong .tuul li").length - 1));
								$("#chuantong .tuul").animate(
									{
										"left":-1140 * ($("#chuantong .tuul li").length - 2)
									}
								,500);
								//小圆点跟上
								$("#chuantong .dianul li").eq(nowimg).addClass("cur").siblings().removeClass("cur");
				...
				$("#chuantong .dianul li").click(
					function(){
						...
						if(dian < nowimg){
							...
							//此时，打动一个身位
							$("#chuantong .tuul").animate({"left":-1140 * (nowimg -1)},500,function(){
								//拉动完毕一个身位之后，我们要将真的图片替换这张假图
								$("#chuantong .tuul").css("left",-1140 * dian);
