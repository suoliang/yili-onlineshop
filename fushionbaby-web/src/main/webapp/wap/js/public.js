$(window).load(function() {
  ...
  // -------------  address.html
  // 单选按钮交互
  $('#address .hide_radio_click').click(function(){
    $('.list_view_item').removeClass('checked');
  ...
  // 按钮交互
  $('#car .hide_radio_click').click(function(){
  ...
  $(".required").blur(function() {
    ...
    if ($(this).is(".user_input")) {
      ...
      } else if (this.value.length == 11 && this.value != "" && /^\d*$/.test(this.value)) {
        success();
        $(this).tooltip('hide');
      } else {
        error();
    ...
    if ($(this).is(".phone_input")) {
      // 验证手机号(11位数字)
      if (this.value.length == 11 && this.value != "" && /^\d*$/.test(this.value)) {
        success();
        $(this).tooltip('hide');
      } else {
        error();
