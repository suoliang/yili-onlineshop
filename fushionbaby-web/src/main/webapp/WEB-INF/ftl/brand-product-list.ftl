...
<head>
	<meta charset="UTF-8">
	<title><#if skuBrand.brandDesc?exists && skuBrand.brandDesc !=''>${skuBrand.brandDesc}<#else>Fushionbaby 兜世宝 母婴用品_一站式海外进口母婴专业潮流用品_100%正品保证</#if></title>
	...
	<div class="toy container">
		<div class="toy-nav">
			<div class="toy-nav-l">				
				...
				<span style="font-size:14px"><#if skuBrand?exists && skuBrand.brandDesc?exists>${skuBrand.brandDesc}</#if></span>
			</div>
			<ul class="toy-nav-r">
				<li>
					<span>排列顺序</span>
					<img src="${rc.contextPath}/views/images/dot.png" alt="">
					<a href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&sort_param=${SortTypeConstant.MOST_NEW}">最新</a>
				</li>
				<li>
					<img src="${rc.contextPath}/views/images/sale.png" alt="">
					<a href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&sort_param=${SortTypeConstant.SALES_VOLUME}">销量</a>
				</li>
				<li>
					<img src="${rc.contextPath}/views/images/price.png" alt="">
					<a href="">价格</a>
					<a href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&sort_param=${SortTypeConstant.PRICE}&sort_type=${SortTypeConstant.desc}" class="tip">
					...
					<a href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&sort_param=${SortTypeConstant.PRICE}&sort_type=${SortTypeConstant.asc}" class="tip">
		...
		<div class="toy-bd container">
			...
			<#assign rows=(pageset.skuList?size-1)/FtlConstant.PRODUCT_BRAND_COLS+1 />
			<#assign cols=FtlConstant.PRODUCT_BRAND_COLS>
			<#list 1..rows as row>
			...
			<div class="collect-page-wrap clearfix">
				<div class="collect-page">			
					...
					<#if pageset.skuList?exists && (pageset.skuList?size > 0)	>
						...
						<#if previous gt 0> 
							<a href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&cur_page=${pageset.curPage-1}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-arrow iconfont page_prev"></a>
						<#else>
							...
							<#list (pageset.curPage)..(pageset.curPage+limit-1) as i >
								<#if i <= pageset.totalPage  >
									...
									<#if i == pageset.curPage>
										<a href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&cur_page=${i?c}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-list page-list-current">${i}</a>
									<#else>
										<a href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&cur_page=${i?c}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-list">${i}</a>
									</#if>
								</#if>
							</#list>
						<#else>
						...
						<#if last <= pageset.totalPage>
							 <a	href="${rc.contextPath}/product/productListByBrand.do?brandId=${skuBrand.id?c}&cur_page=${pageset.curPage+1}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-arrow iconfont page_next">&#xe604;</a>
						<#else>
