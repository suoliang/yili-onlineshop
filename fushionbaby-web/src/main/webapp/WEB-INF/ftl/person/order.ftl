...
<head>
	<meta charset="UTF-8">
	<title>fushionbaby 母婴用品【全部订单】</title>
	...
	<style>
		.all{color: #0089A8;}
		.pay{background-position: -34px -81px;}
	</style>
	<!--自定义-->
...
<body>
				...
				<dd>
					<div class="order-info">
						<a class="pay-wrap" href="javascript:gotoAllOrder();">
							...
							<p class="order-info-desc all">全部订单</p>
						</a>
					</div>
				</dd>
				<dd>
						...
						<tr class="order-ft">
							<td colspan="7">
								...
								<#elseif order.order_status == '5' || order.order_status == '7'>
									...
									<div class="order-ft-r">
										<button class="pay-btn con-rec-btn" style="cursor: pointer;" onclick="confirmAllOrder(${order.code})">确定收货</button>
										<div class="act-pay">
											<span class="amount">实付款：</span>
											<span class="tot">&yen;</span>
											<span class="tot">${order.total_actual}</span>
								...
								<#elseif order.order_status == '6' || order.order_status == '8' || order.order_status == '9' || order.order_status == '10' || order.order_status == '11'>
									<div class="order-ft-l">
										...
										<div class="del-order" onclick="delOrder(${order.id})">
											<i></i>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$(".if2",parent.document).addClass('list-current');
		});	
		<!--自定义-->
		function delOrder(orderId){
			$("#order_code").val(orderId);
		}
		<!--确认收货-->
		function confirmAllOrder(orderCode){
