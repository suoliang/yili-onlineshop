...
<head>
	<meta charset="UTF-8">
	<title>fushionbaby 母婴用品【待收货】</title>
	...
	<style>
		.order-info .sended {background-position: -34px -157px; color: #0089A8;}
	</style>
	<!--自定义-->
...
<body>
	<div class="order-main">
							...
							<td colspan="7">
								<div class="order-ft-l">
									<div class="time-wrap">
										<div class="acc-time">
											<span>配送信息:</span>
											<span class="dispatch-info">${order.trans_name}</span>
											<span class="ml">快递单号：</span>
											<span class="dispatch-info">${order.trans_code}</span>
										</div>
									</div>
								</div>
								<div class="order-ft-r">
									<button class="pay-btn con-rec-btn" style="cursor: pointer;" onclick="confirmOrder(${order.code})">确定收货</button>
									<div class="act-pay">
		...
		<input type="hidden" id="confirmOrder_id"/> 
		<input type="hidden" id="order_type" value="${type}">
	</div>
	<div class="collect-page-wrap ord-det clearfix">
		<div class="collect-page">			
			<#if pageset?exists && pageset.orderList?exists && (pageset.orderList?size > 0)	>
						...
						<#if previous gt 0> 
							<a href="${rc.contextPath}/membercenter/receive.do?cur_page=${pageset.curPage-1}&sid=${sid}&time=${time?c}" class="page-arrow iconfont"></a>
						<#else>
							...
							<#list (pageset.curPage)..(pageset.curPage+limit-1) as i >
								<#if i <= pageset.totalPage  >
									...
									<#if i == pageset.curPage>
										<a href="${rc.contextPath}/membercenter/receive.do?cur_page=${i?c}&sid=${sid}&time=${time?c}" class="page-list page-list-current">${i}</a>
									<#else>
										<a href="${rc.contextPath}/membercenter/receive.do?cur_page=${i?c}&sid=${sid}&time=${time?c}" class="page-list">${i}</a>
									</#if>
