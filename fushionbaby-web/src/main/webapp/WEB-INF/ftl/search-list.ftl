...
<head>
	<meta charset="UTF-8">
	<title>fushionbaby 兜世宝 母婴用品【搜索内容】</title>
		...
		<div class="toy-nav">
			...
			<ul class="toy-nav-r">
				<li>
					...
					<a href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&sort_param=${SortTypeConstant.MOST_NEW}">最新</a>
				</li>
				<li>
					<img src="${rc.contextPath}/views/images/sale.png" alt="">
					<a href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&sort_param=${SortTypeConstant.SALES_VOLUME}">销量</a>
				</li>
				<li>
					<img src="${rc.contextPath}/views/images/price.png" alt="">
					<a href="">价格</a>
					<a href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&sort_param=${SortTypeConstant.PRICE}&sort_type=${SortTypeConstant.desc}" class="tip">
					...
					<a href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&sort_param=${SortTypeConstant.PRICE}&sort_type=${SortTypeConstant.asc}" class="tip">
		...
		<div class="toy-bd container">
					...
					<#else>
						...
						<span class="item-info">
								...
								<span class="item-desc">
									<a href="${rc.contextPath}/product/skuDetail.do?skuId=${sku.id?c}" target="_blank" >
										<#if sku.description?exists>${sku.description}</#if>
									</a>
								</span>
								<img src="${rc.contextPath}/views/images/newg.png" alt="" class="tag">
							</div>
						</#if>	
					</#list>	
			...
			<div class="collect-page-wrap clearfix">
				<div class="collect-page">			
					...
					<#if previous gt 0> 
						<a href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&cur_page=${pageset.curPage-1}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-arrow iconfont page_prev"></a>
					<#else>
						...
						<#list (pageset.curPage)..(pageset.curPage+limit-1) as i >
							<#if i <= pageset.totalPage  >
								<#if i == pageset.curPage>
									<a href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&cur_page=${i?c}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-list page-list-current">${i}</a>
								<#else>
									<a href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&cur_page=${i?c}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-list">${i}</a>
								</#if>
							</#if>
						</#list>
					<#else>
					...
					<#if last <= pageset.totalPage>
						 <a	href="${rc.contextPath}/product/search-list.do?searchKey=${searchKey?url}&cur_page=${pageset.curPage+1}&sort_param=${pageset.sortParam}&sort_type=${pageset.sortType}" class="page-arrow iconfont page_next">&#xe604;</a>
					<#else>
