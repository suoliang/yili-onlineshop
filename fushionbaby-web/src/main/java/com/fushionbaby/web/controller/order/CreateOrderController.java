	...
	public Object createOrder(OrderParamUser orderParam, HttpServletRequest request) {
		try {
			...
			if(StringUtils.equals(OrderConstant.GOTO_CART_PAYMENT, orderParam.getGotoType())){
				List<ShoppingCartSkuUser> cartItemList = soShoppingCartSkuWebService.
						findSelectedCartSkuByMemberId(user.getMemberId());
			...
			if(StringUtils.equals(OrderConstant.GOTO_CART_PAYMENT, orderParam.getGotoType())){
			...
			}else if(StringUtils.equals(OrderConstant.GOTO_IMMEDIATE_PAYMENT, orderParam.getGotoType())){
				...
				createOrderFacade.saveOrderLineByCombination(orderParam);
				createOrderFacade.updateInventoryByCombination(orderParam, user);
				createOrderFacade.saveOrderCombination(orderParam);
			}
			...
			createOrderFacade.modifyMemberAccountBalance(orderParam, user);
