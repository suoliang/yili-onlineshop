...
public class ShoppingCartAddController {
	...
	private static final Log LOGGER = LogFactory
			.getLog(ShoppingCartAddController.class);
	...
	public Object addToCart(String skuCode, String quantity,
		...
		try {
			...
			if (!StringUtils.equals(
					resultCode = CartUtils.checkSidAndVisitKey(request),
			...
			if (!StringUtils.equals(
					resultCode = CartUtils.checkQuantity(quantity),
					CommonCode.SUCCESS_CODE)) {
				return Jsonp.newInstance(resultCode,
						ShoppingCartCaptChaEnum.getTitle(resultCode));
			}
			if (!StringUtils.equals(
					resultCode = addFacade.checkSkuStatus(skuCode),
					CommonCode.SUCCESS_CODE)) {
				return Jsonp.newInstance(resultCode,
						ShoppingCartCaptChaEnum.getTitle(resultCode));
			}
			...
			if (!StringUtils.equals(
					resultCode = addFacade.checkSkuInventoryNum(quantityNum,
							skuCode), CommonCode.SUCCESS_CODE)) {
				return Jsonp.newInstance(resultCode,
						ShoppingCartCaptChaEnum.getTitle(resultCode));
			}
			...
			if (user != null) {
				...
				if (!StringUtils.equals(checkResult.getResponseCode(),
						CommonCode.SUCCESS_CODE)) {
					return checkResult;
				}
				Jsonp addResult = addFacade.hasLoginCartAdd(shoppingCartBo);
				if (!StringUtils.equals(addResult.getResponseCode(),
						CommonCode.SUCCESS_CODE)) {
					return Jsonp.error();
				}
	...
	public Object cartSkuCount(HttpServletRequest request) {
		...
		if (StringUtils.isBlank(vkey) && StringUtils.isBlank(sid)) {
			return Jsonp.paramError("参数获取异常");
		}
		UserDto user = null;
