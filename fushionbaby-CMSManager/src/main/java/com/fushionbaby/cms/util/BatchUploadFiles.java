...
import com.fushionbaby.cms.opdic.SpringContextHolder;
...
public class BatchUploadFiles {
	...
	//private static final Double UPIMAGE_SIZE = 500.0; // 上传限制大小为500
	...
	private static void getParamMap(Map<String, String> paramMap,
			UploadResult result, String absPath, String remotePath) {
		File file = new File(absPath);
		// 判断上传类型
		List<String> types = Arrays.asList(IMAGE_ARRAY);
		String fi = file.getName();
		String name = fi.substring(fi.indexOf(".") == -1 ? 0
				: fi.indexOf(".") + 1, fi.length());
		// 存储上传文件本地名
	...
	public static List<UploadResult> BatchUpload(FileInfo info)
			throws Exception {
		List<UploadResult> results = new ArrayList<UploadResult>();
		List<String> allFiles = new ArrayList<String>();

		getAllFiles(allFiles, info, results);
	...
	public static List<UploadResult> BatchUpload(List<String> allFileAbs,
			String remotePath) {
		List<UploadResult> results = new ArrayList<UploadResult>();
		Map<String, String> paramMap = FTPUtils.getParamMap();
	 ...
	 * @param results
	 */
	public static void deleteSucImage(List<UploadResult> results) {
		for (UploadResult result : results) {
			if (Objects.equals(result.getIsSuccess(), Constant.UPLOAD_SUCCESS)) {
				File file = new File(result.getLoaclRootPath()
						+ result.getFileNameBefore());
				file.delete();
			}
		}
	}
	...
	public static void getAllFiles(List<String> results, FileInfo info,List<UploadResult> upResults) {
			...
			for (int i = 0; i < fs.length; i++) {
				...
				if (fs[i].isFile()) {
					if (info.getTypes().indexOf(name.toLowerCase()) < 0) {
						upResults.add(new UploadResult(Constant.UPLOAD_FAILURE,
								Constant.UPLOAD_FORMAT_ERROR, fs[i]
										.getAbsolutePath(), fi));
					} else if (ImageUtils.getFileSizes(fs[i]) > image_size) {
						upResults.add(new UploadResult(Constant.UPLOAD_FAILURE,
								String.format(Constant.UPLOAD_SIZE_ERROR,
										image_size + "KB"), fs[i]
										.getAbsolutePath(), fi));
					} else {
						results.add(fs[i].getAbsolutePath().toLowerCase());
					}
				}

				if (fs[i].isDirectory()) {
					info.setRootPath(fs[i].getAbsolutePath());
					getAllFiles(results, info, upResults);
				}
			}
		} catch (Exception e) {
	...
	public static boolean delFtpImage(String fileName, String remoteDir) {
		Map<String, String> paramMap = FTPUtils.getParamMap();
		getParamMap(paramMap, fileName, remoteDir);

		return FTPUtils.deleteFile(paramMap);
	}
	...
	public static void main(String[] args) {
		...
		// File file = new File("e:\\images\\123456789123456\\card1.jpg");
		// file.delete();
