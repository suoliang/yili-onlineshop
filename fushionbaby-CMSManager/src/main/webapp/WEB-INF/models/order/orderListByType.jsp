...
<html lang="zh-CN">
    <head>
        ...
        <script type="text/javascript">
			...
			function refund(paymentType,memberId,orderCode){
				$.ajax({
	 				   type: "GET",
	 							...
	 							var submit =  function(v,h,f){
	 					  			if(v=="ok"){
	 					  				if(paymentType=='ZFB_APP'){
	 					  					...
	 					  					location.href="${contextPath}/wxRefund/wxRefund/"+memberId+"/"+orderCode;
	 					  				}else if(paymentType=='ZXYL_APP'){
	 					  					location.href="${contextPath}/unionRefund/unionRefund/"+memberId+"/"+orderCode;
	 					  				}
                    ...
                    <div class="panel panel-info">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="fa fa-shopping-cart"></i> 
                                <c:if test="${orderBaseDto.orderType == '1'}">
					                                                   正常单列表 
					            </c:if>
					            <c:if test="${orderBaseDto.orderType == '2'}">
					                                                    测试单列表
					            </c:if>
					            <c:if test="${orderBaseDto.orderType == '3'}">
					             	    问题单列表
					            </c:if>
                            </h3>
                        </div>
                                ...
                                <table class="table table-bordered table-hover" id="roleTable">
                                    ...
                                    <tbody>
										...
										<%-- 			<c:if test="${order.paymentType=='ZFB_APP' && order.financeStatus =='y' && order.orderStatus != '12' }">
														<a class="btn btn-info btn-xs"  href="javascript:void(0);" onclick="javascript:refund('${order.paymentType}',${order.memberId},'${order.orderCode}')" title="退款">	
															支付宝退款
														</a>
													</c:if>
