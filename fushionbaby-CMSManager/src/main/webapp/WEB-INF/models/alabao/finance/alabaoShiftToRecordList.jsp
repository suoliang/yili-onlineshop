                ...
                <div class="col-md-12 content">
                    <div class="panel panel-info">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="fa fa-user"></i>转入记录 </h3>
                        </div>
                        <div class="panel-body">
                            <form class="form-inline page" id="findForm" method="post">
                                <div class="form-group col-md-4 mB15">
                                    <label for="a" class="col-label">转出类型：</label>
                                	<select name="shiftToAccountType" class="form-control lg-select" data-placeholder="Choose a Category" tabindex="1">
	                                	<option value="">全部</option>
									    <c:forEach items="${dic:getDictByType('SHIFT_TO_ACCOUNT_TYPE')}" var="dict">
									       <option value="${dict.value }"  <c:if test="${dict.value==shiftToAccountType }">selected</c:if> >${dict.label }</option>
									    </c:forEach>
                                	</select>
...
<!-- 											<th>操作</th> -->
                                    ...
                                    <c:forEach items="${page.result}" var="list" varStatus="status">
                                    	<tr>
											...
											<td>${dic:getDictLabel(list.shiftToAccountType, 'SHIFT_TO_ACCOUNT_TYPE', '-')}</td>
